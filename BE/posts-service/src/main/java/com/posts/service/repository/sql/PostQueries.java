...
public class PostQueries {
	...
	public static final String GET_BY_POST_ID = "SELECT p.id, p.user_name, p.caption, p.\"location\", p.tags, p.created_at, ui.profile_image "
			+ "FROM auth.posts p LEFT JOIN  auth.user_info ui ON p.user_name = ui.user_name WHERE p.id = :postId";
	public static final String GET_FILES_BY_POST_ID = "SELECT id, url, post_id "
			+ "FROM auth.files WHERE post_id=:postId";
	public static final String GET_RECENT_POSTS = "SELECT p.id, p.user_name, p.caption, p.location, p.tags, p.created_at, p.updated_at, ui.profile_image, "
			+ "f.id AS file_id, f.url AS file_url, f.post_id " + "FROM auth.posts p "
			+ "LEFT JOIN auth.files f ON p.id = f.post_id LEFT JOIN auth.user_info ui ON p.user_name = ui.user_name "
	...
	public static final String EXPLORE = "SELECT DISTINCT p.id, p.user_name, p.caption, p.location, p.tags, p.created_at, p.updated_at,  "
			...
			+ "FROM auth.posts p " + "LEFT JOIN auth.likes l ON p.id = l.post_id "
			+ "LEFT JOIN auth.files f ON p.id = f.post_id LEFT JOIN "
	...
	public static final String SEARCH = "SELECT DISTINCT p.id, p.user_name, p.caption, p.location, p.tags, p.created_at, p.updated_at,  "
			+ "                f.id AS file_id, f.url AS file_url, ui.profile_image " + "FROM auth.posts p "
